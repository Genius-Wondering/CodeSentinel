# DevAssistant / CodeSentinel — 开发日志

记录开发过程中遇到的实际问题、排查思路、以及每个设计决策背后的原因。按时间顺序记录，阶段一到阶段九对应从最初的代码审查修复到最后一次架构演进，粗糙但真实——包括做错的判断、推翻过的方案、和承认没跑通的部分。

**标签说明：** `[BUG]` 运行时缺陷　`[设计]` `[设计决策]` 单点设计取舍　`[架构]` `[架构决策]` 涉及技术选型/系统边界的决定　`[问题发现]` 发现但当时未必能立刻解决的现象　`[安全]` 安全相关　`[环境]` 跨平台/依赖环境问题　`[待解决]` `[TODO]` 已知但暂未处理的局限　`[新增]` `[移除]` `[前端]` `[后端]` 功能性变更

如果只想快速了解我在这个项目里是怎么做技术判断的，建议直接看下面"按类型快速跳转"里的**架构与设计决策**部分，那几条是最能体现取舍思路的。

---

## 目录

- [阶段一：CodeSentinel 代码审查与修复](#阶段一codesentinel-代码审查与修复)
  - [[BUG] tree-sitter Parser 初始化方式错误](#bug-tree-sitter-parser-初始化方式错误)
  - [[BUG] 行号是 0-based，但引用是 1-based](#bug-行号是-0-based但引用是-1-based)
  - [[BUG] Chroma `add_documents` 无批量分片，大仓库会超限](#bug-chroma-add_documents-无批量分片大仓库会超限)
  - [[BUG] 空集合上执行 `similarity_search` 报内部错误](#bug-空集合上执行-similarity_search-报内部错误)
  - [[安全] GitHub Token 注入用字符串替换，存在边缘情况](#安全-github-token-注入用字符串替换存在边缘情况)
  - [[设计] 文件路径存绝对路径导致索引不可迁移](#设计-文件路径存绝对路径导致索引不可迁移)
  - [[设计] 模块导入时实例化 Agent，影响测试和冷启动](#设计-模块导入时实例化-agent影响测试和冷启动)
  - [[次要] 其他小问题](#次要-其他小问题)
- [阶段二：架构重新审视](#阶段二架构重新审视)
  - [[架构] `document_lifecycle_manager.py` 为什么不属于这个项目](#架构-document_lifecycle_managerpy-为什么不属于这个项目)
  - [[架构决策] ChromaDB vs FAISS 的本质区别，以及为什么最初的对比是错的](#架构决策-chromadb-vs-faiss-的本质区别以及为什么最初的对比是错的)
  - [[架构决策] 为什么保留 Streamlit 而不是只做 MCP](#架构决策-为什么保留-streamlit-而不是只做-mcp)
- [待解决的问题](#待解决的问题)
- [阶段三：多文档结构处理 + 混合索引](#阶段三多文档结构处理-混合索引)
  - [[问题发现] 按字符数切块会破坏文档结构](#问题发现-按字符数切块会破坏文档结构)
  - [[问题发现] 向量搜索找不到精确的变量名和标识符](#问题发现-向量搜索找不到精确的变量名和标识符)
  - [[设计决策] BM25 的 corpus 放内存，重启后 fallback 到纯向量](#设计决策-bm25-的-corpus-放内存重启后-fallback-到纯向量)
  - [[设计决策] `source_type` 元数据贯穿整个 pipeline](#设计决策-source_type-元数据贯穿整个-pipeline)
  - [[待解决] BM25 在 cross-search 场景下权重可能需要动态调整](#待解决-bm25-在-cross-search-场景下权重可能需要动态调整)
- [阶段四：依赖整理、环境搭建与新增问题修复](#阶段四依赖整理环境搭建与新增问题修复)
  - [[环境] Windows GBK 编码导致启动失败](#环境-windows-gbk-编码导致启动失败)
  - [[环境] LangChain 0.x → 1.x 迁移：导入路径全面失效](#环境-langchain-0x-1x-迁移导入路径全面失效)
  - [[BUG] `requirements.txt` 与 `pyproject.toml` 严重不一致](#bug-requirementstxt-与-pyprojecttoml-严重不一致)
  - [[BUG] `/stats` 端点重启后 `code_chunks`/`doc_chunks` 始终为 0](#bug-stats-端点重启后-code_chunksdoc_chunks-始终为-0)
  - [[次要] `repo_loader.py` 异常静默丢失，排查困难](#次要-repo_loaderpy-异常静默丢失排查困难)
- [待解决的问题（延续）](#待解决的问题延续)
- [阶段五：运行时 Bug 修复（跑通验证）](#阶段五运行时-bug-修复跑通验证)
  - [[次要] 两个小 Bug](#次要-两个小-bug)
  - [本阶段测试覆盖](#本阶段测试覆盖)
- [阶段六：文档上传功能（v0.3.0）](#阶段六文档上传功能v030)
  - [功能概述](#功能概述)
  - [[后端] DocChunker 扩展：支持 TXT / DOCX / XLSX](#后端-docchunker-扩展支持-txt-docx-xlsx)
  - [[后端] 新增 `POST /api/v1/index/upload` 接口](#后端-新增-post-apiv1indexupload-接口)
  - [[后端] `app/services/indexing.py` 扩展](#后端-appservicesindexingpy-扩展)
  - [[后端] `app/config.py` 新增配置项](#后端-appconfigpy-新增配置项)
  - [[前端] 全面重构（v0.3.0）](#前端-全面重构v030)
  - [本阶段测试覆盖](#本阶段测试覆盖-1)
  - [新增依赖总结](#新增依赖总结)
- [阶段七：Advanced RAG 全链路升级（v0.4.0）](#阶段七advanced-rag-全链路升级v040)
  - [背景：基础 RAG 的四个缺陷](#背景基础-rag-的四个缺陷)
  - [Stage 1 — 检索前优化（Pre-Retrieval Optimization）](#stage-1-检索前优化pre-retrieval-optimization)
  - [Stage 2 — 多路召回（Multi-Route Retrieval）](#stage-2-多路召回multi-route-retrieval)
  - [Stage 3 — 重排序（Re-ranking）](#stage-3-重排序re-ranking)
  - [Stage 4 — 上下文融合（Context Fusion）](#stage-4-上下文融合context-fusion)
  - [Stage 5 — 生成后反馈循环（Post-Generation Feedback Loop）](#stage-5-生成后反馈循环post-generation-feedback-loop)
  - [数据流对比](#数据流对比)
  - [新增/修改文件清单](#新增修改文件清单)
  - [新增配置项](#新增配置项)
  - [本阶段测试覆盖](#本阶段测试覆盖-2)
- [阶段八：主线功能精简（移除非核心模块，2026-06）](#阶段八主线功能精简移除非核心模块2026-06)
  - [[移除] GitHub 一键克隆索引 (`/index/github`)](#移除-github-一键克隆索引-indexgithub)
  - [[移除] `document_lifecycle_manager.py`](#移除-document_lifecycle_managerpy)
  - [[移除] `notes.md`、`scripts/`](#移除-notesmdscripts)
  - [改动后的项目结构](#改动后的项目结构)
  - [本阶段验证](#本阶段验证)
- [阶段九：持久化元数据层 + 意图路由 + 公开 Benchmark 评测（v0.5.0，2026-06）](#阶段九持久化元数据层-意图路由-公开-benchmark-评测v0502026-06)
  - [[新增] 持久化元数据层](#新增-持久化元数据层)
  - [[新增] 意图路由（`app/agent/intent_routing.py`）](#新增-意图路由appagentintent_routingpy)
  - [[新增] SWE-QA + RAGAS 端到端答案质量评测（`tests/swe_qa_ragas_eval.py`）](#新增-swe-qa-ragas-端到端答案质量评测testsswe_qa_ragas_evalpy)
  - [[新增] BEIR 检索器泛化能力评测（`tests/beir_retriever_eval.py`）](#新增-beir-检索器泛化能力评测testsbeir_retriever_evalpy)
  - [依赖管理](#依赖管理)
  - [本阶段验证汇总](#本阶段验证汇总)
...
按问题类型而非时间顺序索引，方便直接跳到某一类条目（同一条目在上面的目录里也能找到，这里只是换一个查找维度）。

**架构与设计决策**
- [[设计] 文件路径存绝对路径导致索引不可迁移](#设计-文件路径存绝对路径导致索引不可迁移)　_阶段一_
- [[设计] 模块导入时实例化 Agent，影响测试和冷启动](#设计-模块导入时实例化-agent影响测试和冷启动)　_阶段一_
- [[架构] `document_lifecycle_manager.py` 为什么不属于这个项目](#架构-document_lifecycle_managerpy-为什么不属于这个项目)　_阶段二_
- [[架构决策] ChromaDB vs FAISS 的本质区别，以及为什么最初的对比是错的](#架构决策-chromadb-vs-faiss-的本质区别以及为什么最初的对比是错的)　_阶段二_
- [[架构决策] 为什么保留 Streamlit 而不是只做 MCP](#架构决策-为什么保留-streamlit-而不是只做-mcp)　_阶段二_
- [[设计决策] BM25 的 corpus 放内存，重启后 fallback 到纯向量](#设计决策-bm25-的-corpus-放内存重启后-fallback-到纯向量)　_阶段三_
- [[设计决策] `source_type` 元数据贯穿整个 pipeline](#设计决策-source_type-元数据贯穿整个-pipeline)　_阶段三_

**安全与健壮性**
- [[安全] GitHub Token 注入用字符串替换，存在边缘情况](#安全-github-token-注入用字符串替换存在边缘情况)　_阶段一_

**Bug 排查与环境问题**
- [[BUG] tree-sitter Parser 初始化方式错误](#bug-tree-sitter-parser-初始化方式错误)　_阶段一_
- [[BUG] 行号是 0-based，但引用是 1-based](#bug-行号是-0-based但引用是-1-based)　_阶段一_
- [[BUG] Chroma `add_documents` 无批量分片，大仓库会超限](#bug-chroma-add_documents-无批量分片大仓库会超限)　_阶段一_
- [[BUG] 空集合上执行 `similarity_search` 报内部错误](#bug-空集合上执行-similarity_search-报内部错误)　_阶段一_
- [[次要] 其他小问题](#次要-其他小问题)　_阶段一_
- [[环境] Windows GBK 编码导致启动失败](#环境-windows-gbk-编码导致启动失败)　_阶段四_
- [[环境] LangChain 0.x → 1.x 迁移：导入路径全面失效](#环境-langchain-0x-1x-迁移导入路径全面失效)　_阶段四_
- [[BUG] `requirements.txt` 与 `pyproject.toml` 严重不一致](#bug-requirementstxt-与-pyprojecttoml-严重不一致)　_阶段四_
- [[BUG] `/stats` 端点重启后 `code_chunks`/`doc_chunks` 始终为 0](#bug-stats-端点重启后-code_chunksdoc_chunks-始终为-0)　_阶段四_
- [[次要] `repo_loader.py` 异常静默丢失，排查困难](#次要-repo_loaderpy-异常静默丢失排查困难)　_阶段四_
- [[次要] 两个小 Bug](#次要-两个小-bug)　_阶段五_

**问题发现与未解决项**
- [[问题发现] 按字符数切块会破坏文档结构](#问题发现-按字符数切块会破坏文档结构)　_阶段三_
- [[问题发现] 向量搜索找不到精确的变量名和标识符](#问题发现-向量搜索找不到精确的变量名和标识符)　_阶段三_
- [[待解决] BM25 在 cross-search 场景下权重可能需要动态调整](#待解决-bm25-在-cross-search-场景下权重可能需要动态调整)　_阶段三_

**功能迭代（新增/移除/前后端）**
- [[后端] DocChunker 扩展：支持 TXT / DOCX / XLSX](#后端-docchunker-扩展支持-txt-docx-xlsx)　_阶段六_
- [[后端] 新增 `POST /api/v1/index/upload` 接口](#后端-新增-post-apiv1indexupload-接口)　_阶段六_
- [[后端] `app/services/indexing.py` 扩展](#后端-appservicesindexingpy-扩展)　_阶段六_
- [[后端] `app/config.py` 新增配置项](#后端-appconfigpy-新增配置项)　_阶段六_
- [[前端] 全面重构（v0.3.0）](#前端-全面重构v030)　_阶段六_
- [[移除] GitHub 一键克隆索引 (`/index/github`)](#移除-github-一键克隆索引-indexgithub)　_阶段八_
- [[移除] `document_lifecycle_manager.py`](#移除-document_lifecycle_managerpy)　_阶段八_
- [[移除] `notes.md`、`scripts/`](#移除-notesmdscripts)　_阶段八_
- [[新增] 持久化元数据层](#新增-持久化元数据层)　_阶段九_
- [[新增] 意图路由（`app/agent/intent_routing.py`）](#新增-意图路由appagentintent_routingpy)　_阶段九_
- [[新增] SWE-QA + RAGAS 端到端答案质量评测（`tests/swe_qa_ragas_eval.py`）](#新增-swe-qa-ragas-端到端答案质量评测testsswe_qa_ragas_evalpy)　_阶段九_
- [[新增] BEIR 检索器泛化能力评测（`tests/beir_retriever_eval.py`）](#新增-beir-检索器泛化能力评测testsbeir_retriever_evalpy)　_阶段九_
...
- `_parse_fallback` 的 docstring 写在了赋值语句之后，Python 不会报错（不在函数体第一句就只是无用的字符串字面量），但 review 时容易被误读成"还没生效的默认值逻辑"，调整了顺序。
- `app/main.py` 里 `FastAPI(version="0.2.1")` 和 root 端点返回的 `{"version": "0.2.0"}` 不同步，`/docs` 和 `/` 显示的版本号不一致，没有功能影响但容易在 debug 时造成困惑。更好的做法是统一到 `config.py` 里的一个 `VERSION` 常量（后续处理）。
...
索引仓库后 `indexed_chunks` 数量比预期少，但没有任何报错。原因是文件读取的两处异常捕获（`OSError`、`UnicodeDecodeError`）直接 `continue`，不记录日志，权限或编码问题导致的文件跳过完全不可见。修复：加 `logging.getLogger(__name__)`，跳过文件时在 `WARNING` 级别记录原因和路径。
...
**Sidebar 索引数量始终显示 0：** `frontend/app.py` 读 `/stats` 时用的键名是 `indexed_chunks`，但接口实际返回的键名是 `total_chunks`（阶段四已统一），两端键名不一致导致 `get()` 取不到值。顺带把展示内容升级为分类统计：
```python
st.caption(f"Indexed: **{stats.get('total_chunks', 0)}** chunks "
           f"(code: {stats.get('code_chunks',0)}, doc: {stats.get('doc_chunks',0)})")
```

**`/health` 返回错误的服务名：** 项目从 DevAssistant 改名为 CodeSentinel 时这一处硬编码的 `"service": "DevAssistant"` 漏改了，改为与全局一致的 `"CodeSentinel"`。
